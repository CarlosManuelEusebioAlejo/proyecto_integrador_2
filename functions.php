...
function obtener_post($post_por_pagina, $conexion) {
    ...
    $sentencia = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM publicaciones WHERE status = 'aprobado' ORDER BY fecha DESC LIMIT $inicio, $post_por_pagina");
    $sentencia->execute();
    return $sentencia->fetchAll();
}

function obtener_notificaciones($conexion, $usuario){
    $sentencia = $conexion->prepare("SELECT * FROM publicaciones WHERE id_usuario = '$usuario' AND (status = 'aprobado' OR status = 'rechazado') ORDER BY fecha DESC");
